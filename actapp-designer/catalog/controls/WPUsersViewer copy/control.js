(function (ActionAppCore, $) {
  ...
  ControlCode._onInit = function () {
    ...
    // this.parts.mainform.controlConfig.index.items['submit-bar'].classes = 'hidden';
    // this.parts.mainform.controlConfig.index.items['title'].classes = 'hidden';
  ...
  ControlCode.refreshSelection = function () {
    ...
    //---> Use this method of we are use _deleted again
    // var tmpDocCount = 0;
    // for (var iPos in this.tableData) {
    //   var tmpDoc = this.tableData[iPos];
  ...
  ControlCode.newDoc = function () {
    ...
    ThisApp.apiCall(tmpURL).then(function (theReply) {
      ...
      if (theReply && theReply.data && theReply.data.length) {
        ...
        self.parts.mainform.prompt({ title: 'Add User', submitLabel: 'Save New User', doc: tmpDoc }).then(function (theSubmit, theData) {
          if (!theSubmit) { return; }
